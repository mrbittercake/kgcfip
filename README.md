...
一个帮你自建 Cloudflare 优选 IP 库的小工具，告别万人坑，掌控每一条网络连接！ 🚀

---

## ✨ 这玩意儿和其它工具的区别？

老是用别人分享的优选 IP？是不是经常碰到速度慢、或者用的人太多被墙了的破事？这个工具就是为了解决这些烦恼而生的。跟其他公开的测速工具比，它有几个核心的好处：

-   **纯网页，超方便**: 不用下载和安装任何客户端软件。只要有浏览器就能用，手机、电脑随时随地开测，管理你的 IP。
-   **打造你自己的私人 IP 库**: 别再用那些公开的 IP 池了，人多眼杂，说不定哪天就被大佬玩坏了。用这个工具，测出来的都是你自己的，干净、高速，只为你一个人服务。再也不怕 IP 被滥用导致连不上或被封。
-   **不同网络，一键切换**: 🏠🏢📱 家里、公司、手机流量，网络环境不一样，好用的 IP 也不一样。你可以给不同场景建个分组，比如“公司摸鱼专用”、“回家看片专用”。用的时候点一下就能换上最快的 IP，美滋滋。
-   **订阅链接，自动更新**: 每个分组都能生成一个订阅链接。把这个链接扔进其他大佬的工具里（自己去了解，本项目不做分享），以后就不用手动换 IP 了。这边测速更新了，那边自动就用上最新的，懒人福音。

所有测速数据、IP 列表和配置都存在你自己的 Cloudflare KV 里面，别人谁也看不见，安全得很。


## 部署教程 (Deployment Guide)

本项目推荐使用 GitHub + Cloudflare Pages 的方式进行全自动部署。🚀 你只需要将代码推送到你的 GitHub 仓库，Cloudflare 就会自动完成构建和部署，非常方便。
...
-   等待部署流程走完，看到绿色的 `Success`，你的私人优选 IP 管理系统就上线啦！
